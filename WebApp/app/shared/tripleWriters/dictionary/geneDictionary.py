def getGenes():
    return allGenes, cgfGenes
def getAllGenes():
    return allGenes
def getAGenes():
    return allGenes[allGenes.index("Asp"):allGenes.index("Unc (atpA)")]
def getMLSTGenes():
    return allGenes[allGenes.index("Unc (atpA)"):allGenes.index("MOMP")+1]
allGenes = ['Asp', 'Gln', 'Glt', 'Gly', 'Pgm', 'Tkt', 'Unc (atpA)', 'flaA SVR', 'flaPeptide', 'porA', 'MOMP']
