...
# GLOBAL VARIABLES
####################################################################################################
CAMPY = "https://github.com/campy-db/Campy-Database/blob/master/Ontologies/CampyOntology.owl#"
LAB = "https://github.com/campy-db/Campy-Database/blob/master/Ontologies/LabTests.owl#"
...
def createAMRTriple(isoTitle, data, rpattern=""):
    print("AMR TRIPLE TEST>>>>>>>>>>>>>>")
    def mic_triple(mic_value, drug, test_title):
        dm_title = "{}_{}".format(mic_value, drug);

    triple = ltm.untTriple(dm_title, "DrugMIC") + \
             ltem
...
def createEnviroTriple(data, iso_title):
    ...
    print("e_prop: " + data["enviro_prop"])

    enviro = data["enviro"]
    print("enviro: " + data["enviro"])
