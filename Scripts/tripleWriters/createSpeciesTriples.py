...
def createSpeciesTriples(df, row, isoTitle):
    ...
    # lethSpec is the default species unless it is empty,  equal to 'other campylobacter', or just
    # some invalid value. In this case alt spec becomes the species,  granted it has a valid value.
    # Sometimes the species are mixed (of the form 'Mixed (coli and jejuni)') in which case
    # specA = "coli" and specB = "jejuni". But sometimes it will just say 'Mixed' in lethSpec but
    # 'Coli' in altSpec. In such a case specA = "coli" and specB remains empty

    lethIsValid = not pd.isnull(lethSpec) and cn.isGoodVal(lethSpec) and\
              not cn.compare([lethSpec, "dead"])

    altIsValid = \
    not pd.isnull(altSpec) and cn.isGoodVal(altSpec) and not cn.compare([altSpec, "dead"])
