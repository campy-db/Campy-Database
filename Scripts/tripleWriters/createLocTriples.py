...
"""
 ...
 Gets all the info of where we got the isolate sample, eg country, province etc.
...
def createLocTriples(df, row, isoTitle):
    ...
    # Petting zoo is in this col, and we consider it to be a sampling site.
    samplingSite2 = df["Source_Specific_2"][row]
    ...
    if not pd.isnull(water) and "Water" in water:
        ...
        if not pd.isnull(bodyOfWater):
            ...
            if "Watershed" in bodyOfWater:
                ...
                locTriple +=\
                st.addStandardTrips(isoTitle, "hasSourceLocation", bodyOfWater, "Watershed")
                ...
                if re.search("[Rr]iver", bodyOfWater) is not None:
                    ...
                    locTriple +=\
                    st.addStandardTrips(isoTitle, "hasSourceLocation", bodyOfWater, "Body_of_water")
